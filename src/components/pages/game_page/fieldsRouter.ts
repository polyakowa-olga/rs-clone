  ...
  protected static commonRoute(player: IPlayer, field: ICardsData) {
    ...
    switch (true) {
      case [6, 25].includes(fieldId):
        ...
        const forceMajorBtn = document.createElement(
          "button"
        ...
        forceMajorBtn.addEventListener("click", () => {
          ...
          if (!isEnough) {
            ...
            chat.run(Game.chatWindowBox, player, field, undefined, "sold"); ///// chat
            return;
          }
      ...
      case [8, 27].includes(fieldId):
        ...
        message = FieldsRouter.MessageDataGetter.chanceMessages();////// chat

        chat.run(Game.chatWindowBox, player, field, message);///// chat
      ...
      case [13].includes(fieldId):
        ...
        chat.run(Game.chatWindowBox, player, field); ////// chat
  ...
  protected static tradeRoute(player: IPlayer, field: ICardsData) {
    switch (field.owner) {
      ...
      case null:
        ...
        buyBtn.addEventListener("click", () => {
          if (player.money >= fieldPrice) {
            ...
            chat.run(Game.chatWindowBox, player, field, undefined, "buy");////// chat
            ...
            // console.log(
            //   `Player ${player.id} doesn't have enough in cash to buy ${field.title}`
            // );
            SoundsGame.AdminSound();

            chat.run(Game.chatWindowBox, player, field, undefined, "nobuy");////// chat
      ...
      default:
        ...
        const currFieldValue = field.currValue
          ? field.currValue
          : (field.price as number);
        const sumToPay = [7, 26].includes(field.id)
          ? currFieldValue * GameCubeRoll.sum
        ...
        payBtn.addEventListener("click", () => {
          ...
          const isBankrupt = PlayerCash.payPlayer2Player(
            player,
            sumToPay,
            field
          );
          ...
          chat.run(Game.chatWindowBox, player, field, undefined, "payto");////// chat
